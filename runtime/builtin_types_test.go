...
func TestBuiltinFuncs(t *testing.T) {
	...
	cases := []struct {
		...
		{f: "reversed", args: wrapArgs(NewList()), want: NewList().ToObject()},
		{f: "reversed", args: wrapArgs(newTestList("bar", "foo")), want: newTestList("foo", "bar").ToObject()},
		{f: "reversed", args: wrapArgs(newTestList(false, true)), want: newTestList(true, false).ToObject()},
		{f: "reversed", args: wrapArgs(newTestList(3, 0, 1, 2)), want: newTestList(2, 1, 0, 3).ToObject()},
		{f: "reversed", args: wrapArgs(newTestRange(5)), want: newTestList(4, 3, 2, 1, 0).ToObject()},
		{f: "reversed", args: wrapArgs(1), wantExc: mustCreateException(TypeErrorType, "'int' object is not iterable")},
		{f: "reversed", args: wrapArgs(newTestList("foo", "bar"), 2), wantExc: mustCreateException(TypeErrorType, "'reversed' requires 1 arguments")},
