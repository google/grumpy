...
func TestBuiltinFuncs(t *testing.T) {
	...
	cases := []struct {
		...
		{f: "tuple", args: wrapArgs(newTestList(1, "foo", 1.0)), want: NewTuple(NewInt(1).ToObject(), NewStr("foo").ToObject(), NewFloat(1.0).ToObject()).ToObject()},
		{f: "tuple", args: wrapArgs(newTestDict()), want: NewTuple().ToObject()},
		{f: "tuple", args: wrapArgs(0), wantExc: mustCreateException(TypeErrorType, "'int' object is not iterable")},
		{f: "tuple", args: wrapArgs(newObject(badIterType)), wantExc: mustCreateException(RuntimeErrorType, "foo")},
