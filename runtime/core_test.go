...
func TestBinaryOps(t *testing.T) {
	...
	cases := []struct {
		...
		{IPow, NewInt(2).ToObject(), NewInt(2).ToObject(), NewInt(4).ToObject(), nil},
		{IPow, newObject(ObjectType), newObject(fooType), nil, mustCreateException(TypeErrorType, "unsupported operand type(s) for |: 'object' and 'Foo'")},
		...
		{Pow, NewInt(2).ToObject(), NewInt(2).ToObject(), NewInt(4).ToObject(), nil},
		{Pow, newObject(ObjectType), newObject(fooType), nil, mustCreateException(TypeErrorType, "unsupported operand type(s) for |: 'object' and 'Foo'")},
